...
export default function Home() {
  ...
  return (
    <div className="w-full h-auto p-[2%] text-white mt-[6%]">
      <Hero />
      <div className="w-full h-[50vh] flex p-[2%] items-center justify-evenly gap-[1%] py-[3%] mt-[4%]">
        {dock.map((dock) => (
          <motion.div   initial={{opacity:0.4,y:200}} animate={{opacity:1, y:0,transitionDuration:"0.4s",transition:"ease-in-out"}}  className="p-[1%] flex flex-col bg-gray-700 rounded-[25px] shadow-2xl shadow-gray-600 border" style={{height:dock.height, width: dock.width}}>
            ...
            <div className="w-full h-[40%] flex flex-col p-[2%] justify-evenly" style={{fontSize:dock.fontsize}}>
              <h2 className="text-shadow-lg text-shadow-gray-950">{dock.name}</h2>
              <p className="text-shadow-lg text-shadow-gray-950">{dock.title}</p>
            </div>      
          </motion.div>
        ))}
      </div>

      <motion.div initial={{opacity:0.4,y:200}} animate={{opacity:1, y:0,transitionDuration:"0.4s",transition:"ease-in-out"}} className="w-full h-[85vh] border rounded-[25px] shadow-2xl shadow-gray-600 bg-[url('public/Img/pink-cyan.jpg')] bg-cover bg-center border-black flex overflow-hidden">
        <div className="w-[30%] h-full flex flex-col items-center justify-center pl-[6%]">
          <h1 className="text-[90px] text-shadow-xl">RTX 4070</h1>
          <p className="text-[30px] text-shadow-xl">Nvidia Collection </p>
          <p className="text-shadow-xl">Est. 2024</p>
        </div>
